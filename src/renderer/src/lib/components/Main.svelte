<script lang="ts">
  ...
  onMount(() => {
    ...
    window.addEventListener('desktop:webui-integrations-opened', closeSettings)
    return () => window.removeEventListener('desktop:webui-integrations-opened', closeSettings)
  })
